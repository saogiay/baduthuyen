<?php

namespace App\Rules;

use Illuminate\Contracts\Validation\Rule;
use Illuminate\Support\Str;

class checkSlug implements Rule
{
    /**
     * Create a new rule instance.
     *
     * @return void
     */
    public function __construct()
    {
        //
    }

    /**
     * Determine if the validation rule passes.
     *
     * @param  string  $attribute
     * @param  mixed  $value
     * @return bool
     */
    public function passes($attribute, $value)
    {
        if ($value) {
            $slug = Str::slug($value);
            $checkPost = \App\Baiviet::where('code', $slug)->first();
            $checkProduct = \App\Sanpham::where('code', $slug)->first();
            $checkCategoryPost = \App\Danhmucbaiviet::where('code', $slug)->first();
            $checkCategoryProduct = \App\Danhmucsanpham::where('code', $slug)->first();
            $check = $checkPost || $checkProduct || $checkCategoryPost || $checkCategoryProduct;

            if ($check) {
                return false;
            } else {
                return true;
            }
        } else {
            return false;
        }
    }

    /**
     * Get the validation error message.
     *
     * @return string
     */
    public function message()
    {
        return 'Code đã tồn tại trong hệ thống.';
    }
}
